// 啟航日 4/7號
// 4/9任務，放上unbound error的檢查
// 4/23全部寫完了*******************，沒想到我的PJ3第9題卡隱測，導致10,12都錯，PJ4的第10題(因為10,12是9的加總版，PJ4是前面的加總版)
// 結果盡然是list? 再拋出(ERROR,"ERROR (incorrect number of arguments) : List?")時我的L是大寫，導致我4/12卡到今天4/23，真的是太笨了。
#include <stdio.h>
#include <iostream>
#include <vector>
#include <string>
  ...
  Token scanToken(){
    ...
    if( c == '(' ){ // ( 左刮要peek下一個是不是右刮)
      ...
      if( nextchar == ')'){
        ...
        return Token(NIL, "nil", sline, scolumn);
      } // if
      ...
      return Token(QUOTE, "quote", sline, scolumn);
    else if( c == '\"'){ // 雙括是字串***********\n要的話要throw error嗎
      bool closed = false;
      while( nextchar != '\n' && nextchar != EOF ){ // 如果遇到換行的話
        char c = getTheChar();
        if ( c == '\"' ){
          closed = true;
          str = str + c ;
      ...
      if( str == "#f" || str == "nil" )
        ...
        return Token(T, "#t", sline, scolumn);
      else
        return Token(SYM, str, sline, scolumn);
    } // else if
...
void prettyPrint(Sexp s,int M){ // 換個位置
  int i = 0, j = 0;
  if ( s.type == LIST || s.type == PAIR){ 
    ...
    if( s.mtype == INT )
      ...
      cout << fixed << setprecision(3) << atof(s.value.c_str()) << endl;
    else if( s.mtype == NIL ) 
      cout << "nil" << endl;
    else if( s.mtype == T ) 
      cout << "#t" << endl;
...
void prettyPrintRead(Sexp s,int M){ // 把prettyPrint遞迴猜開，因為write不用endl
  int i = 0, j = 0;
  if ( s.type == LIST || s.type == PAIR){ 
    cout << "( "; 
    for( i = 0 ; i < s.Slist.size() ; i++ ){ // 後續的都在M+2
    ...
    if( s.mtype == INT )
      cout << atoi(s.value.c_str());
    else if( s.mtype == FLOAT )
      cout << fixed << setprecision(3) << atof(s.value.c_str());
    else if( s.mtype == NIL ) 
      cout << "nil";
    else if( s.mtype == T ) 
      cout << "#t";
...
Sexp predictAtom(const vector<Sexp> & selist  ){ // nil是atom，沒修好bug!!!! 4/8
  ...
  if(  selist[0].type == SYMBOL  || selist[0].type == NUMBER || selist[0].type == STRING_O || 
       selist[0].type == ERROR_OBJECT || selist[0].type == FUNCTION || selist[0].type == FUNCTION_LAMBDA )
    return Sexp(SYMBOL,T,"#t");
  else
    return Sexp(SYMBOL,NIL,"nil");  
} // add
...
Sexp doExit(const vector<Sexp> & selist){ // 這是想到的寫法(把正解寫在計算中，所以跑到這=錯誤的level)
  throw Sexp(ERROR,"(level of EXIT)"); // 丟錯誤物，所以不會到這個跟下面
} // doExit
...
bool expectedFunctionArgcNum( Sexp fun,int argcSize ) {
  if( fun.value == "+" || fun.value == "-" || fun.value == "*" || fun.value == "/" || fun.value == ">" 
    ...
    if( argcSize < 2)
      ...
      return true;
           ...
           || fun.value == "create-error-object" ||  fun.value == "error-object?" || fun.value == "write"
           || fun.value == "display-string" || fun.value == "eval" || fun.value == "symbol->string"
  ...
  else if( fun.value == "newline" || fun.value == "read" ) {
    if( argcSize != 0)
      return false;
    else  
      return true;
...
Sexp predictVerbose(const vector<Sexp> & selist  ){
  if( selist.size() != 0   )
    throw Sexp(ERROR,"ERROR (incorrect number of arguments) : verbose?"); //
  if( verboseSignal == 1  )
    return Sexp(SYMBOL,T,"#t");
...
// project4 function
Sexp createErrObject(const vector<Sexp> & selist  ){
  if( selist.size() != 1   )
    throw Sexp(ERROR,"ERROR (incorrect number of arguments) : create-error-object"); //
  if ( selist[0].type == LIST || selist[0].type == PAIR )
    throw Sexp(ERROR,"ERROR (create-error-object with incorrect argument type) :", selist[0].Slist);
  else if( selist[0].mtype != STRING ) // 早知道不分這麼多了，.....
    throw RebuildSexp("ERROR (create-error-object with incorrect argument type) : ",selist[0]); // 丟錯誤物件
  // string跟error都可以重定向，所以要做的其實好像只有改個type
  return Sexp(ERROR_OBJECT,STRING,selist[0].value); 
} // add

Sexp PredictErrObject(const vector<Sexp> & selist  ){
  if( selist.size() != 1   )
    throw Sexp(ERROR,"ERROR (incorrect number of arguments) : error-object?"); //
  if( selist[0].type == ERROR_OBJECT )
    return Sexp(SYMBOL,T,"#t");
  else
    return Sexp(SYMBOL,NIL,"nil"); 
} // add

Sexp doRead(const vector<Sexp> & selist  ){
  if( selist.size() != 0   )
    throw Sexp(ERROR,"ERROR (incorrect number of arguments) : read"); //
  try{ //這邊要重構一下read，所以把scanner，parser變成全域了。
    g_parser.startParsing();
    Sexp s = g_parser.getSexp();
    return s;
  } // try
...
Sexp doWrite(const vector<Sexp> & selist  ){ //同上同上
  if( selist.size() != 1   )
    throw Sexp(ERROR,"ERROR (incorrect number of arguments) : write"); //
  prettyPrintRead(selist[0],0);
  
  return selist[0];
} // add

Sexp doEval(const vector<Sexp> & selist  ){
  if( selist.size() != 1   )
    throw Sexp(ERROR,"ERROR (incorrect number of arguments) : eval"); 
  return Sexp();
} // add

Sexp displayString(const vector<Sexp> & selist  ){
  if( selist.size() != 1   )
    throw Sexp(ERROR,"ERROR (incorrect number of arguments) : display-string"); //
  if ( selist[0].type == LIST || selist[0].type == PAIR )
    throw Sexp(ERROR,"ERROR (number->string with incorrect argument type) :", selist[0].Slist);
  else if( selist[0].mtype != STRING ) // 早知道不分這麼多了，.....
    throw RebuildSexp("ERROR (number->string with incorrect argument type) : ",selist[0]); // 丟錯誤物件
  string str = selist[0].value.substr(1,selist[0].value.size()-2); // 如果是""的話，應該是
  cout << str;
  return selist[0];
} // add

Sexp newLine(const vector<Sexp> & selist  ){
  if( selist.size() != 0   )
    throw Sexp(ERROR,"ERROR (incorrect number of arguments) : newline"); //
  cout << endl;
  return Sexp(SYMBOL,NIL,"nil"); 
} // add

Sexp symbolToString(const vector<Sexp> & selist  ){
  if( selist.size() != 1   )
    throw Sexp(ERROR,"ERROR (incorrect number of arguments) : symbol->string"); //
  if ( selist[0].type == LIST || selist[0].type == PAIR )
    throw Sexp(ERROR,"ERROR (number->string with incorrect argument type) :", selist[0].Slist);
  else if( selist[0].mtype != SYM ) // 早知道不分這麼多了，.....
    throw RebuildSexp("ERROR (number->string with incorrect argument type) : ",selist[0]); // 丟錯誤物件 
  string str;  
  str = "\"" + selist[0].value + "\"";  
  return Sexp(STRING_O, STRING, str);
...
Sexp numberToString(const vector<Sexp> & selist  ){
  if( selist.size() != 1   )
    throw Sexp(ERROR,"ERROR (incorrect number of arguments) : number->string"); //
  if ( selist[0].type == LIST || selist[0].type == PAIR )
    throw Sexp(ERROR,"ERROR (number->string with incorrect argument type) :", selist[0].Slist);
  else if( selist[0].type != NUMBER )
    throw RebuildSexp("ERROR (number->string with incorrect argument type) : ",selist[0]); // 丟錯誤物件 
  ...
  stringstream ss;
  if( checkIsINT(selist) == true )
    ss << atoi(selist[0].value.c_str()); // 防止+5,-5，加號要88
  else // 是float取3位
    ss << fixed << setprecision(3) << atof(selist[0].value.c_str());
  str = ss.str();  
  str = "\"" + str + "\"";  
  return Sexp(STRING_O,STRING,str);
} // numberToString
...
class Environ{ //設計是這是變數的放置區，
  public:  
    ...
    void buildinFunction(){
      ...
      // 4/11
      env["verbose"] = Sexp(FUNCTION, verboseMod, "verbose"); //
      env["verbose?"] = Sexp(FUNCTION, predictVerbose, "verbose?"); // 
      // PROJECT4
      env["create-error-object"] = Sexp(FUNCTION, createErrObject, "create-error-object"); // 
      env["error-object?"] = Sexp(FUNCTION, PredictErrObject, "error-object?"); // 
      env["read"] = Sexp(FUNCTION, doRead, "read"); // 
      env["write"] = Sexp(FUNCTION, doWrite, "write"); // 
      env["eval"] = Sexp(FUNCTION, doEval, "eval"); // 同quote
      env["display-string"] = Sexp(FUNCTION, displayString, "display-string"); // 
      env["newline"] = Sexp(FUNCTION, newLine, "newline"); // 
      env["symbol->string"] = Sexp(FUNCTION, symbolToString, "symbol->string"); // 
      env["number->string"] = Sexp(FUNCTION, numberToString, "number->string"); // 
    ...
    void doTheGlobalDefine(string str, Sexp arg){
      Environ *temp =  this;
      if( temp->Parent != NULL ){ //沒找到找往global，parent往上找。
    ...
    void updateEnv( string str, Sexp arg ){
      ...
      if( lambdaLayer == true){
        ...
        return temp->updateEnv(str,arg);  
      } // if

      if( Parent != NULL ){ //沒找到找innerEnv
  ...
  public:
    ...
    Sexp evaluting( Environ * inEnv, Sexp ss ){  // 3/29改掉在這一層try-catch會使錯誤在遞迴時被接收導致收不到錯誤。
      ...
      if( ss.mtype == NIL || ss.mtype == T || ss.mtype == STRING || ss.type == NUMBER || ss.type == ERROR
                          || ss.type == FUNCTION  || ss.type == FUNCTION_LAMBDA || ss.type == ERROR_OBJECT ) // 寫PROJECT 1以為這些也可以變形，難受了，不知道要不要FUNCTION
        return ss; 
        ...
        if( temp.mtype == QUOTE ) // QUOTE就拿下一個Sexp
          ...
          if( ss.Slist[1].type == SYMBOL ){ // 定義SYMBOL
            ...
            if( ss.Slist[2].type == ERROR && ( tempArgment.type == LIST || tempArgment.type == PAIR )  ) // 複製之前的結果 4/9
              throw Sexp(ERROR,"ERROR (no return value) : ", tempArgment.Slist); 
            else if( ss.Slist[2].type == ERROR ){
              tempArgment.type = ERROR;
              throw RebuildSexp("ERROR (no return value) : ", tempArgment); // 丟錯誤物件 
            } // else if

            inEnv->updateEnv(ss.Slist[1].value,ss.Slist[2]);
            if( verboseSignal != 0 ) // " defined"
              cout << ss.Slist[1].value + " defined" << endl;
            return Sexp(ERROR,""); // define不回傳value
            //印出a defined;
          } // if

          else if( ss.Slist[1].type == LIST ){ // 定義FUNCTION (define (a b) 2) => (define a ( lambda (b) 2) ) 要改語法
            if( ss.Slist[1].Slist.size() == 0 || ss.Slist[1].Slist[0].type != SYMBOL || ( isReservedWord( ss.Slist[1].Slist[0].value )) ){ // 應該不會有這種狀況存在
              throw Sexp(ERROR,"ERROR (DEFINE format) : ", ss.Slist); //碰到保留字了 不知道要回傳list還是錯誤的保留字
            } // if
            ...
            inEnv->updateEnv(ss.Slist[1].Slist[0].value, changetoLambdaExpr);
            if( verboseSignal != 0 ) // " defined"
              cout << changetoLambdaExpr.value + " defined" << endl;
            return Sexp(ERROR,""); // define不回傳value
        ...
        else if( temp.value == "lambda" ){ // lambda這個有夠難處理，計畫中打算先讓  他變成function然後要呼叫他時把，list to body，並加上args
          ...
          if( isReservedWord(ss.Slist[1].value) ){
            throw Sexp(ERROR,"ERROR (SET! format) : ", ss.Slist); //碰到保留字了 不知道要回傳list還是錯誤的保留字
          } // if
         
          Sexp tempArgment = ss.Slist[2]; 
          ss.Slist[2] = evaluting( inEnv, ss.Slist[2]);
          if( ss.Slist[2].type == ERROR && ( tempArgment.type == LIST || tempArgment.type == PAIR )  ) // 先寫PJ4的內容
            throw Sexp(ERROR,"ERROR (no return value) : ", tempArgment.Slist); 
          else if( ss.Slist[2].type == ERROR ){
            tempArgment.type = ERROR;
            throw RebuildSexp("ERROR (no return value) : ", tempArgment); // 丟錯誤物件  
          ...
          for( int i = 1 ; i < ss.Slist.size() ; i++){
            ...
            if( tempNode.type == ERROR && ( ss.Slist[i].Slist[0].type == LIST || ss.Slist[i].Slist[0].type == PAIR )  )
              throw Sexp(ERROR,"ERROR (unbound test-condition) : ", ss.Slist[i].Slist[0].Slist);  
            else if( tempNode.type == ERROR ){
              ss.Slist[i].Slist[0].type = ERROR;
              ss.Slist[i].Slist[0].value = "ERROR (unbound test-condition) : "  + ss.Slist[i].Slist[0].value ;
              throw ss.Slist[i].Slist[0]; 
            } // else if
          ...
          if( tempNode.type == ERROR && ( ss.Slist[1].type == LIST || ss.Slist[1].type == PAIR )  )
            throw Sexp(ERROR,"ERROR (unbound test-condition) : ", ss.Slist[1].Slist);  
          else if( tempNode.type == ERROR ){
            ss.Slist[1].type = ERROR;
            ss.Slist[1].value = "ERROR (unbound test-condition) : "  + ss.Slist[1].value ;
            throw ss.Slist[1]; 
          } // else if
          ...
          if( tempNode.mtype != NIL){
            ...
            if( ss.Slist.size() == 4 ){
              ...
              return Sexp(ERROR,"ERROR (no return value) : ", ss.Slist);
          } // else
        } // else if 
          ...
          for( int i = 0 ; i < ss.Slist[1].Slist.size() ; i++){ // 看測資來說好像要先檢查，所以先讓他空跑一次了
          ...
          for( int i = 0 ; i < ss.Slist[1].Slist.size() ; i++){ // ( ( symbol1 sexp1 )(sym2 sexp2) ...) 
            ...
            Sexp tempArgment = ss.Slist[1].Slist[i].Slist[1]; 
            ss.Slist[1].Slist[i].Slist[1] = evaluting(inEnv,ss.Slist[1].Slist[i].Slist[1]);
            if( ss.Slist[1].Slist[i].Slist[1].type == ERROR && ( tempArgment.type == LIST || tempArgment.type == PAIR )  ) // 先寫PJ4的內容
              throw Sexp(ERROR,"ERROR (no return value) : ", tempArgment.Slist); 
            else if( ss.Slist[1].Slist[i].Slist[1].type == ERROR ){
              tempArgment.type = ERROR;
              throw RebuildSexp("ERROR (no return value) : ", tempArgment); // 丟錯誤物件  
            } // else if
            
            SexpValues.push_back( ss.Slist[1].Slist[i].Slist[1] );
          } // for
          ...
          for( int i = 1 ; ss.Slist.size() ; i++){
            ...
            if( tempArgment.type == ERROR && ( ss.Slist[i].type == LIST || ss.Slist[i].type == PAIR )  )
              throw Sexp(ERROR,"ERROR (unbound condition) : ", ss.Slist[i].Slist);  
            else if( tempArgment.type == ERROR ){
              ss.Slist[i].type = ERROR;
              ss.Slist[i].value = "ERROR (unbound condition) : "  + ss.Slist[i].value ;
              throw ss.Slist[i]; 
            } // else if
            
            if( tempArgment.mtype == NIL)
              return Sexp(SYMBOL,NIL,"nil");
        ...
        if( temp.type == ERROR && ( ss.Slist[0].type == LIST || ss.Slist[0].type == PAIR )  )
          throw Sexp(ERROR,"ERROR (no return value) : ", ss.Slist[0].Slist);  
        else if( temp.type == ERROR ){
          ss.Slist[0].type = ERROR;
          ss.Slist[0].value = "ERROR (no return value) : "  + ss.Slist[0].value ;
          return ss.Slist[0]; 
        } // else if
        ...
        if( temp.type == FUNCTION ){
          ...
          if( expectArgument == false ){
            ...
            else if( temp.value == "eval"){
              Sexp tempArgement = evaluting( inEnv, ss.Slist[1] );
              level = 0;
              return evaluting( inEnv, tempArgement );
            } // else if
            ...
            if( ss.Slist.size() > 1 ){
              for(int i = 1 ; i < ss.Slist.size() ; i++ ){
                ...
                tempArgment = evaluting(inEnv, ss.Slist[i]);
                if( tempArgment.type == ERROR && ( ss.Slist[i].type == LIST || ss.Slist[i].type == PAIR )  )
                  throw Sexp(ERROR,"ERROR (unbound parameter) : ", ss.Slist[i].Slist);  
                else if( tempArgment.type == ERROR ){
                  ss.Slist[i].type = ERROR;
                  ss.Slist[i].value = "ERROR (unbound parameter) : "  + ss.Slist[i].value ;
                  throw ss.Slist[i]; 
                } // else if
  
                argc.push_back(tempArgment);
              } // for
            } // if
          } // else
          ...
          for( int i = 0 ; i < temp.Slist.size() ; i++ ){ // 4/7pj3第三題 忘記了lambda只回傳最後了
              if( i == temp.Slist.size()-1 ){
                ...
                if( tempArgment.type == ERROR && ( temp.Slist[temp.Slist.size()-1].type == LIST || temp.Slist[temp.Slist.size()-1].type == PAIR )  )
                  return Sexp(ERROR,"ERROR (no return value) : ", ss.Slist);  
                else if( tempArgment.type == ERROR ){ // 可能是會有錯的地方
                  temp.Slist[temp.Slist.size()-1].type = ERROR;
                  temp.Slist[temp.Slist.size()-1].value = "ERROR (no return value) : "  + temp.Slist[temp.Slist.size()-1].value ;
                  return temp.Slist[temp.Slist.size()-1]; 
                } // else if

                return tempArgment; // 4/7pj3第三題 要計算size-1才對(因為lambda可能有多項)
                } // if
    ...
    void project2(Environ *globalglobal ,int &endsignal){ // 3/29把try catch移入這一層
      ...
      if( ce.type == ERROR ){ //為什麼要unbound parameter可以吃掉no return value error。 
        ...
        else if( ce.type == ERROR && ce.value.size() > 0 ) // 特別處理處理正常型別
          cout << ce.value  << endl;
        return;
      } // if
...
int main(){
  ...
  g_scanner = new Scanner(); // 因為架構寫太爛了，導致把scanner和parser寫成全域的
  g_parser = Parser(g_scanner);  // 同上
  Evaluate evaluate;
  ...
  while( true ){
    try {
      cout << endl << "> ";
      g_parser.startParsing();
      evaluate.setSexp(g_parser.getSexp());
      // if( utestnum == 2){
      //    break;
      // }
