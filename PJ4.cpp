...
Sexp predictAtom(const vector<Sexp> & selist  ){ // nil是atom，沒修好bug!!!! 4/8
  ...
  if(  selist[0].type == SYMBOL  || selist[0].type == NUMBER || selist[0].type == STRING_O || selist[0].type == ERROR )
    return Sexp(SYMBOL,T,"#t");
  else
    return Sexp(SYMBOL,NIL,"nil");  
} // add
...
class Evaluate {
  public:
    ...
    void project2(Environ *globalglobal ,int &endsignal){ // 3/29把try catch移入這一層
      ...
      if( ce.value.size() > 8 &&  ( ce.value.substr(ce.value.size()-8, ce.value.size()) == " defined" ) && verboseSignal == 0 ) // " defined"
        return;
      prettyPrint(ce,0);
    } // project1
