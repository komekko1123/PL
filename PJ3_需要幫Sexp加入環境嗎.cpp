// 啟航日 4/7號
// 4/9任務，放上unbound error的檢查，lambda的大麻煩( ( Flambda -10 ) x1 ) ; error測資，( Flambda -10 )這串結果是lambda，要保存結果，但是環境不保存，所以要改sexpression了。
#include <stdio.h>
#include <iostream>
#include <vector>
#include <string>
...
public:
  ...
  Sexp(SexpType Stype, vector<string> parameter, Environ *env){ //傳參數時順便建立lambda
    type = Stype;
    paras = parameter;
    uniqueID = nextID++;
    mtype = NOP;
    ...
    Sexp evaluting( Environ * inEnv, Sexp ss ){  // 3/29改掉在這一層try-catch會使錯誤在遞迴時被接收導致收不到錯誤。
      ...
      if( ss.mtype == NIL || ss.mtype == T || ss.mtype == STRING || ss.type == NUMBER || ss.type == ERROR
        ...
        if( temp.mtype == QUOTE ) // QUOTE就拿下一個Sexp
          ...
          if( ss.Slist[1].type == SYMBOL ){ // 定義SYMBOL
            ...
            if( ss.Slist[2].type == ERROR && ( tempArgment.type == LIST || tempArgment.type == PAIR )  ) // 複製之前的結果 4/9
              return Sexp(ERROR,"ERROR (no return value) : ", tempArgment.Slist); 
            else if( ss.Slist[2].type == ERROR ){
              tempArgment.type = ERROR;
              return RebuildSexp("ERROR (no return value) : ", tempArgment); // 丟錯誤物件  
            } // else if
            ...
            Sexp changetoLambdaExpr = Sexp(FUNCTION_LAMBDA,parameters,inEnv);
        ...
        if( temp.type == FUNCTION ){
          ...
          Environ * localVar = inEnv->extendEnv(temp.innerLambdaEnv,temp.paras,argc); // 放入LocalVariable去定義
          for( int i = 0 ; i < temp.Slist.size() ; i++ ){ // 4/7pj3第三題 忘記了lambda只回傳最後了
              if( i == temp.Slist.size()-1 ){
                ...
                return tempArgment; // 不能回傳tempArgment!!!!!
              } // if
